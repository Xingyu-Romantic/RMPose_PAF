...
from paddle.optimizer.lr import ReduceOnPlateau as ReduceLROnPlateau
...
def train_factory(args, preprocess, target_transforms):
    ...
    train_data = paddle.io.ComposeDataset(train_datas)
...
def cli():
    ...
    if not args.disable_cuda and paddle.is_compiled_with_cuda():
        args.device = paddle.get_device()
...
def get_loss(saved_for_loss, heat_temp, vec_temp):
    ...
    saved_for_log['max_ht'] = paddle.max(
        saved_for_loss[-1].data[:, 0:-1, :, :]).item()
    saved_for_log['min_ht'] = paddle.min(
        saved_for_loss[-1].data[:, 0:-1, :, :]).item()
    saved_for_log['max_paf'] = paddle.max(saved_for_loss[-2].data).item()
    saved_for_log['min_paf'] = paddle.min(saved_for_loss[-2].data).item()
...
for i in range(20):
    for param in model.sublayers()[0].model0[i].parameters():
        param.requires_grad = False

trainable_vars = [param for param in model.parameters() if param.trainable]
optimizer = paddle.optimizer.Momentum( parameters = trainable_vars, learning_rate=args.lr,
...
for epoch in range(5, args.epochs):
    ...
    if is_best:
        paddle.save(model.state_dict(), model_save_filename)      