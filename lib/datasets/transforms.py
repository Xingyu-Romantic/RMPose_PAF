...
class RandomApply(BaseTransform):
    ...
    def __init__(self, lambd = None , probability = 0.1, keys=None):
        super(RandomApply, self).__init__(keys)
        self.prob = probability
        self.transforms = lambd
...
class RandomGrayscale(BaseTransform):
    ...
    def __init__(self, p=0.1, keys=None):
...
normalize = paddle.vision.transforms.Normalize(  # pylint: disable=invalid-name
    mean=[0.485, 0.456, 0.406],
    std=[0.229, 0.224, 0.225]
)


image_transform = paddle.vision.transforms.Compose([  # pylint: disable=invalid-name
    paddle.vision.transforms.ToTensor(),
    normalize,
])


image_transform_train = paddle.vision.transforms.Compose([  # pylint: disable=invalid-name
    paddle.vision.transforms.ColorJitter(brightness=0.1,
                                       contrast=0.1,
                                       saturation=0.1,
                                       hue=0.1),
    RandomApply([
        # maybe not relevant for COCO, but good for other datasets:
        Lambda(jpeg_compression_augmentation),
    ], probability=0.1),
...
class RandomApply(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        if paddle.rand([1]) > self.probability:
...
class RescaleRelative(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        ...
        if isinstance(self.scale_range, tuple):
            scale_factor = (
                self.scale_range[0] +
                paddle.rand([1]) * (self.scale_range[1] - self.scale_range[0])
            )
        else:
...
class RescaleAbsolute(Preprocess):
    ...
    def scale(self, image, anns):
        ...
        if isinstance(this_long_edge, (tuple, list)):
            this_long_edge = int(paddle.randint(this_long_edge[0], this_long_edge[1], (1,)).item())
...
class Crop(Preprocess):
    ...
    def crop(self, image, anns):
        ...
        if w > self.long_edge:
            x_offset = paddle.randint(-padding, w - self.long_edge + padding, (1,))
            x_offset = paddle.clip(x_offset, min=0, max=w - self.long_edge).item()
        if h > self.long_edge:
            y_offset = paddle.randint(-padding, h - self.long_edge + padding, (1,))
            y_offset = paddle.clip(y_offset, min=0, max=h - self.long_edge).item()
...
class CenterPad(Preprocess):
    ...
    def center_pad(self, image, anns):
        ...
        # pad image
        image = paddle.vision.transforms.functional.pad(
