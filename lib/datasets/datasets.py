...
def collate_images_anns_meta(batch):
    images = paddle.io.DataLoader.default_collate([b[0] for b in batch])
...
def collate_multiscale_images_anns_meta(batch):
    ...
    images = [paddle.io.DataLoader.default_collate([b[0][i] for b in batch])
...
def collate_images_targets_meta(batch):
    ...
    images = paddle.io.DataLoader.default_collate([b[0] for b in batch])
    targets1 = paddle.io.DataLoader.default_collate([b[1] for b in batch])
    targets2 = paddle.io.DataLoader.default_collate([b[2] for b in batch])    

    return images, targets1, targets2


class CocoKeypoints(paddle.io.Dataset):
    ...
    def single_image_processing(self, image, anns, meta, meta_init):
        ...
        assert image.shape[2] == original_size[0]
        assert image.shape[1] == original_size[1]
        # mask valid
        ...
        pafs = paddle.to_tensor(pafs.transpose((2, 0, 1)).astype(np.float32))       
...
class ImageList(paddle.io.Dataset):
...
class PilImageList(paddle.io.Dataset):
