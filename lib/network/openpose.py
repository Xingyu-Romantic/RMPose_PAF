...
class OpenPose_Model(nn.Layer):
    ...
    def forward(self, x):
        ...
        for l2_stage in self.l2_stages:
            ...
            x_in = paddle.concat([features, paf_pred], 1)
            paf_ret.append(paf_pred)
        # L1 Stage inference
        for l1_stage in self.l1_stages:
            heat_pred = l1_stage(x_in)
            x_in = paddle.concat([features, heat_pred, paf_pred], 1)
    ...
    def init_w_pretrained_weights(self, pkl_weights='/home/tomas/Desktop/AIFI/internal-repos/aifi-pose/network/weights/openpose/openpose.pkl'):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2D):
                idx = next(conv_idxs)
                m.weight.set_value(paddle.Tensor(weights[idx]['weights'][0]))
                m.bias.set_value(paddle.Tensor(weights[idx]['weights'][1]))
            elif isinstance(m, nn.PReLU):
                idx = next(prelu_idxs)
                m.weight.set_value(paddle.Tensor(weights[idx]['weights'][0]))
...
def use_vgg(model):
    ...
    vgg_state_dict = paddle.load('vgg19-dcbb9e9d.pth')
