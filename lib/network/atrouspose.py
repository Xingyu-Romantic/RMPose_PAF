...
def conv(in_channels, out_channels, kernel_size=3, padding=1, bn=True, dilation=1, stride=1, relu=True, bias=True):
    ...
    if relu:
        modules.append(ReLU_())
    return nn.Sequential(*modules)

class ASPP_ASP(nn.Layer):
    ...
    def forward(self, feature_map):
        ...
        add3 = add2+out_3x3_2
        add4 = add3+out_3x3_3
        out = F.relu(self.bn_out(paddle.concat([add1, add2, add3, add4], 1))) # (shape: (batch_size, 1280, h/16, w/16))
...
class AtrousPose(nn.Layer):
    ...
    def forward(self, x):
        ...
        cat_feat = F.relu(paddle.concat([feature_map, _16x], 1))
...
if __name__ == "__main__":
    ...
    image = paddle.to_tensor(image, dtype='float32').transpose([2,0,1]).reshape(1, 3, 256, 256).cuda()
